...
export const Dropzone: FunctionComponent<DropzoneProps> = ({ error, filesSelected, directoriesSelected, multiple, accept }) => {
    ...
    return <Card background="white" border={error ? "dashed 2px #ea4964" : "dashed 2px #b0b0b0"} minHeight="266px" padding="75px 0" onDragOver={e => {
        ...
        filesSelected(files.map(f => ({ path: f.name, type: f.type, size: f.size })));
    }}>
        {error ?
            <>
                <Text fontSize="18px" fontWeight="bold" color="#ea4964" margin="0" align="center">{error.title}</Text>
                <Text align="center" margin="20px 0" fontSize="0.8em">{error.message}</Text>
            </>
            :
            <>
        ...
        <input type="file" id="fileElem" multiple={multiple} accept={accept || "*/*"} onChange={e => {
            e.preventDefault();
        ...
        </Button>
        {
            ...
            <>
                <input ref={dirFileInputRef} type="file" id="dirElem" multiple={multiple} accept={accept || "*/*"} onChange={e => {
                    ...
                    const files: File[] = [];// = [...e.dataTransfer.files];
                    directoriesSelected(files.map(f => ({ path: f.name, type: "", size: 0 })))
                }} style={({ display: "none" })} />
                <Button color="#0055c4" width="175px" type="button">
